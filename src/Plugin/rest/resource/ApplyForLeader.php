 ...
 *
 * @RestResource(
 *   id = "distribution_apply_for_leader",
 *   label = @Translation("Apply for leader"),
 *   uri_paths = {
 *     "create" = "/api/rest/distribution/apply-for-leader"
 *   }
 * )
 */
class ApplyForLeader extends ResourceBase {
  ...
  public function post($data) {
    ...
    try {
      $distributor = null;
      if ($data['distributor']) $distributor = Distributor::load($data['distributor']);
      if (!$distributor) throw new \Exception('distributor id is required');

      $leader = $this->distributionManager
        ->upgradeAsLeader($distributor, $data);

      return new ModifiedResourceResponse($leader, 200);
